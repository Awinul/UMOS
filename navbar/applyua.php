...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "SELECT * FROM `course` WHERE id='$name'";
    $results = mysqli_query($conn, $sql);
    $datas = $results->fetch_assoc();
    if ($results) {
        $course_name = $datas['cname'];
        $tname = $datas['tname'];
        $zid = $datas['tid'];
        $section = $datas['section'];
        $start = $datas['ctimestart'];
        $end = $datas['ctimeend'];
        $cid = $datas['cid'];
    }
    if ($cgpa > 3.69) {
        ...
        $sp = " SELECT * FROM `apply` WHERE sid='$tid' AND cid='$cid' AND section='$section'";
    ...
    window.open('studenthome.php','studenthome.php');
        ...
        window.open('studenthome.php','studenthome.php');
            ...
            $sql1 = "INSERT INTO `apply`( `sname`, `sid`, `cgpa`, `cname`, `cid`, `section`, `tname`, `ctimestart`, `ctimeend`,tid) VALUES ('$sname','$tid','$cgpa','$course_name','$cid','$section','$tname','$start','$end','$zid')";
            mysqli_query($conn, $sql1);

            echo "<script type='text/javascript'>if (window.confirm('Your Application is Greated'))
    {
    window.open('studenthome.php','studenthome.php');
        ...
        echo "<script type='text/javascript'>if (window.confirm('Your Cgpa or result is not enough for apply'))
    {
    window.open('studenthome.php','studenthome.php');
...
<body class="g-sidenav-show  bg-gray-100">
    ...
    <main class="main-content border-radius-lg ">
        ...
        <form action="" method="POST">
            ...
            <label for="cars">Select Your Subject That you want to apply</label> <br>
            ...
            <select name="cars" id="cars">
                ...
                $sql = "Select * from take_courses where sid='$sid'";
                ...
                while ($row = mysqli_fetch_array($result)) {
                    ...
                    $tname = $row['tname'];
                    $section = $row['section'];
                    $start = $row['ctimestart'];
                    $end = $row['ctimeend'];
                    $cid = $row['cid'];
                    $day = $row['day'];

                    echo "  <option value='$id'>Course:$course_name Teacher Name: $tname Section: $section Day:$day  Class Start: $start Class End:$end</option>";
                }
        ...
        <table class="table">
            ...
            <?php
            $sql = "SELECT * FROM `apply` WHERE sid='$tid'";
            $result = mysqli_query($conn, $sql);
            while ($row = mysqli_fetch_array($result)) {
                $ids = $row['id'];
            ?>
                ...
                <tbody>
                    <tr>
                        ...
                        <td><?php echo "<a href='deleteapply.php?id=$ids'><button type='button' class='btn btn-danger'>Delete</button></a>" ?></td>
                    </tr>
                ...
                <div class="row align-items-center justify-content-lg-between">
                    <div class="col-lg-6 mb-lg-0 mb-4">
                        <div class="copyright text-center text-sm text-muted text-lg-start">
                            <h4>© 2023
                                Build  by
                                <a href="" class="font-weight-bold" target="_blank">Team PathFinders</a>
                            </h4>
