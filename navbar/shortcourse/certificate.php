...
$sq = "SELECT * FROM `shortcourse` WHERE id='$scid'";
$result = mysqli_query($conn, $sq);
$data = $result->fetch_assoc();
if ($result) {
    $_SESSION['caname']=$data['name'];
...
<body class="g-sidenav-show  bg-gray-100">
    ...
    <main class="main-content border-radius-lg ">
        ...
        $totalmark = 0;
        $totalacv = 0;
        $sql = "SELECT DISTINCT(ename) FROM `code` WHERE cid='$scid'";
        $result = mysqli_query($conn, $sql);
        while ($row = mysqli_fetch_array($result)) {
            $totalexam = $totalexam + 1;
        }

        $sql = "SELECT DISTINCT(ename) FROM `mcq` WHERE cid='$scid'";
        $result = mysqli_query($conn, $sql);
        while ($row = mysqli_fetch_array($result)) {
            $totalexam = $totalexam + 1;
        }

        $sql = "SELECT DISTINCT(ename) FROM `participants` WHERE cid='$scid' AND sid='$tid'";
        $result = mysqli_query($conn, $sql);
        while ($row = mysqli_fetch_array($result)) {
            $totalpar = $totalpar + 1;
        }
        ...
        $sql = "SELECT * FROM `mcq` WHERE cid='$scid'";
        $result = mysqli_query($conn, $sql);
        while ($row = mysqli_fetch_array($result)) {
        ...
        $sql = "SELECT * FROM `code` WHERE cid='$scid'";
        $result = mysqli_query($conn, $sql);
        while ($row = mysqli_fetch_array($result)) {
            $totalmark = $totalmark + $row['mark'];
        }
       
        $sql = "SELECT DISTINCT(ename),mark FROM `mark` WHERE cid='$scid' AND sid='$tid'";
        $result = mysqli_query($conn, $sql);
        while ($row = mysqli_fetch_array($result)) {
            $totalacv = $totalacv + $row['mark'];
        }
        ...
        ?>
            ...
            $s = 0;
            $sql = "SELECT  DISTINCT(ename)  FROM `mark` WHERE sid='$tid' ";
            $result = mysqli_query($conn, $sql);
            while ($row = mysqli_fetch_array($result)) {
                $examname = $row['ename'];
                $sql1 = "SELECT  MAX(mark) as mk,sid,week,tmark FROM `mark` WHERE sid='$tid' AND cid='$cid' AND ename='$examname'";
                $result1 = mysqli_query($conn, $sql1);
                while ($row1 = mysqli_fetch_array($result1)) {

                    $tmark = $tmark + $row1['tmark'];
                    $tamark = $tamark + $row1['mk'];
            ?>
                    
            <?php }
            } ?>
        </table>
        <h4>Percentage of mark is <span style="color:blue"><?php
        //echo $tamark;
        //echo $totalmark;

                                                            $per = ($tamark / $totalmark) * 100;

                                                            echo round($per) . "%"; ?></span> </h4>
...
if($totalexam=$totalpar){
    ...
    if($per>80){
        ...
        <a href='mycertificate.php'><h4 style='text-align:center;color:blue;'>Download your certificate</h4></a> 
        
        </div>";
    }
    else{
        
            echo "<h4 style='text-align:center;color:blue;'>Your mark is lass than 80%</h4>";
    }
